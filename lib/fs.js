...
cmds.unzip = function( zip, path ) {
    ...
    .then(function() {
        ...
        return Q.fncall( mods.fs.mkdir, path );
    })
    .then(function() {
        // Unpack the tgz file.
        var cargs = [ zipPath ];
...
cmds.chksum = function( path, cb ) {
    ...
    var stdout = [];
    cksum.stdout.on('data', function( data ) {
        stdout.push( data );
    });
    cksum.stderr.on('data', function( data ) {
        console.log( data.toString() );
    });
    cksum.on('close', function() {
...
cmds.mv = function( from, to, cb ) {
    var dp = Q.defer();
    mods.fs.exists( from, function( exists ) {
        var result;
        if( exists ) {
